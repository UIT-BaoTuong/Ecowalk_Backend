...
public class HomeController {
    ...
    public ResponseEntity<?> getDashboard(@RequestParam Long userId) {
        ...
        for (int i = 6; i >= 0; i--) {
            ...
            String dayKey = d.getDayOfWeek().getDisplayName(TextStyle.SHORT, Locale.ENGLISH); // Mon, Tue...
            distMap.put(dayKey, 0.0);
        }
        
        for (Runs r : recentRuns) {
            String dayKey = r.getStartTime().getDayOfWeek().getDisplayName(TextStyle.SHORT, Locale.ENGLISH);
