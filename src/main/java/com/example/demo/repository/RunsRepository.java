...
public interface RunsRepository extends JpaRepository<Runs, Long> {
  ...
  @Query(
      value =
          "SELECT CAST(rank_val AS INTEGER) FROM ("
              ...
              + ") as ranking_table WHERE id = :userId",
      nativeQuery = true)
  Integer getUserRank(@Param("userId") Long userId, @Param("startDate") LocalDateTime startDate);

  @Query(
      value =
          "SELECT COALESCE(SUM(distance_km), 0) FROM runs WHERE user_id = :userId AND CAST(start_time AS DATE) = CURRENT_DATE",
      nativeQuery = true)
  Double getTodayDistance(@Param("userId") Long userId);

  @Query(
      value =
          "SELECT CAST(COALESCE(SUM(EXTRACT(EPOCH FROM (end_time - start_time))), 0) AS INTEGER) FROM runs WHERE user_id = :userId AND CAST(start_time AS DATE) = CURRENT_DATE",
      nativeQuery = true)
  Integer getTodayTimeSeconds(@Param("userId") Long userId);

  @Query(
      value =
          "SELECT * FROM runs WHERE user_id = :userId AND start_time >= CURRENT_DATE - INTERVAL '7 days'",
      nativeQuery = true)
  List<Runs> getLast7DaysRuns(@Param("userId") Long userId);

  @Query(
      value = "SELECT DISTINCT TO_CHAR(start_time, 'YYYY-MM-DD') FROM runs WHERE user_id = :userId",
      nativeQuery = true)
  List<String> getAllActiveDates(@Param("userId") Long userId);

  @Query(
      value =
          "SELECT TO_CHAR(start_time, 'YYYY-MM-DD'), COALESCE(SUM(distance_km), 0) FROM runs WHERE user_id = :userId GROUP BY TO_CHAR(start_time, 'YYYY-MM-DD')",
      nativeQuery = true)
  List<Object[]> getCalendarStats(@Param("userId") Long userId);

  @Query(
      value =
          "SELECT COALESCE(SUM(distance_km), 0) FROM runs WHERE user_id = :userId AND start_time >= :startDate",
      nativeQuery = true)
  Double getTotalDistanceFrom(
